...
#ursor.execute(categories_table)
...
def change_product(new_country, new_company):
    # Задание №10
    new_info = f"UPDATE produsers SET produser_country='{new_country}', produser_company='{new_company}' WHERE produser_id={SIZE};"
    return new_info
#cursor.execute(change_product('Kyrgyzstan', 'Apple'))
#connection.commit()


# Задание №11
'''info = "SELECT kivano.product_name, kivano.price FROM kivano INNER JOIN sulpak ON sulpak.product_name=kivano.product_name"
cursor.execute(info)
for row in cursor:
    print(row)'''
...
# Задание №12
'''
selected = "SELECT produser_id, produser_country FROM produsers"
...
cursor.execute(f'SELECT produser_company FROM produsers WHERE produser_id={max(x)}')
print(list(cursor))'''

# Задание №13
'''
new_query = "UPDATE sulpak SET price=price+1000"
...
max_price = 0
min_price = 0
cursor.execute("SELECT MAX(sulpak.price), MIN(kivano.price) FROM sulpak, kivano")
