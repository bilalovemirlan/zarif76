...
cursor.execute('''select * from kivano where kivano.product_name not in (select product_name from sulpak);''')
print(cursor.fetchall())
# 7. Найдите все товары в магазине sulpak, где компания-производитель содержит букву "m" в имени.
cursor.execute('''select * from sulpak where product_name LIKE '%m%';''')
print(cursor.fetchall())
# 8. Найдите товары, которые есть и в kivano u sulpak.
cursor.execute('''select * from kivano where kivano.product_name in (select product_name from sulpak);''')
print(cursor.fetchall())
